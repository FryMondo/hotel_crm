...
module.exports = async (req, res) => {
    ...
    try {
        const deleteRoomReservationsQuery = 'DELETE FROM roomreservation WHERE roomNumberID IN (?)';
        await db.query(deleteRoomReservationsQuery, [roomNumbers]);

        const deleteRoomInformationQuery = 'DELETE FROM roominformation WHERE ID IN (SELECT roomInformationID FROM rooms WHERE roomNumber IN (?))';
